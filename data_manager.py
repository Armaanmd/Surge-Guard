...
# --- GLOBAL STORAGE ---
# Persistent log storage for the session
if 'activity_logs' not in globals():
    activity_logs = []
...
def get_stock_alerts(all_stations):
    ...
    for s in all_stations:
        if s['stock'] < 150:
            alerts.append({"level": "ERROR", "msg": f"CRITICAL: {s['brand']} at {s['location']} is almost empty!"})
        elif s['stock'] < 300:
            alerts.append({"level": "WARNING", "msg": f"LOW STOCK: {s['brand']} at {s['location']} is low."})
    return alerts

# --- BOOKING & PREDICTION LOGIC ---
...
def get_priority_requests():
    ...
    for s in sectors:
        ...
        requests.append({
            "Sector": f"{s['icon']} {s['type']}", "Priority": s['priority'],
            "Requested Units": requested, "Allocated Units": allocated, "Gap": requested - allocated
        })
    return requests

# --- LOGGING & CROWDSOURCING ---

def add_log_entry(user, action, details, category="General"):
    entry = {
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "user": user, "action": action, "details": details, "category": category
    }
...
def report_station_traffic(station_name, driver_name, reported_wait):
    return add_log_entry(
        user=f"Driver: {driver_name}",
        action="Live Traffic Report",
        details=f"Reported {reported_wait} mins wait at {station_name}",
        category="Traffic"
    )
